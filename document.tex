...
&\\\hline
&\\
\verb|{ }| &  \Large$\left\{m\atop n\right\}$\normalsize \\ 
&\\\hline
&\\
\verb|\lceil \rceil| & \Large$\left\lceil m\atop n\right\rceil$\normalsize \\ 
&\\\hline
&\\
\verb|\lfloor \rfloor|& \Large$\left\lfloor m\atop n\right\rfloor$\normalsize \\ 
&\\\hline
&\\
\texttt{<  >} & \Large$\left< m\atop n\right>$\normalsize \\ 
&\\\hline
...
se escribe como \verb|\int|. Por ejemplo, el código:  
\begin{verbatim}
$$
\int{x \, dx }
$$
...
\int{x \, dx }
$$
Nótese que es necesario poner \verb|\,| para que se genere
...
$$
\end{verbatim}
generaría el siguiente resultado (nótese el \verb|\,|
...
\begin{lstlisting}[language={[LaTeX]TeX}, caption={Especificación del estilo de
\textit{listings}}, label=lst:styleCode]
\lstset{
    basicstyle=\ttfamily,
    numberstyle=\ttfamily,
    frame=rlbt, % draw a frame at the top and bottom of the code block
