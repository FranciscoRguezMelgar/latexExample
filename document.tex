...
formatos simples, insertas secciones... Y, sobre todo, al principio explicaré
...
eñe, diéresis en la u y demás, no debería haber problemas. Ej.: áéíóú, ÁÉÍÓÚ, üÜ, ñÑ.
...
Si se necesita una función que no esté aquí, por ejemplo digamos una llamada
foo, simplemente se puede usar el comando \verb!\textrm! para que el nombre
de la función se escriba sin estilo matemático, por ejemplo:
\begin{verbatim}
\textrm{foo}\,(x, y)
\end{verbatim}
sería equivale a esto:
$$
\textrm{sen}\,\alpha
$$
nótese que hay que poner <<\verb!\,!>> para que lo que vaya después de la
<<nueva función>> se separe un poco de ella y quede como si fuera una de las de 
\LaTeX{}.
...
La notación que se ha visto para los bloques matemáticos (con signos de dolar)
...
que él mismo
proporciona, llamado del mismo modo. Para ilustrar el código que se realizaría
con ese paquete, se ha creado otro archivo .tex en la raíz del directorio que
...
crear presentaciones de transparencias, utilizando una clase de documento
muy especial llamada \textit{beamer}. Como ya hemos dicho, es muy difícil, si no
imposible, cambiar la clase de un documento cuando éste tiene cierta extensión.
Por lo que se ha creado otro documento llamada \texttt{beamer.tex} que se 
compila con la orden \verb!make beamer.pdf! si se dispone de soporte para
compilar con make. Ese documento será una presentación de transparencias
con distintas características, en este documento explicaré cómo se han creado.

La estructura básica de un documento con la clase beamer es la misma que la de 
uno con cualquier otra clase de documento. Por ejemplo:
\begin{lstlisting}[language={[LaTeX]TeX},
...
transparencia, porque si introducimos mucho, éste se desbordará como se puede
ver en la figura \ref{fig:slide_overflow}
...
serie de salvedades sobre la clase beamer:
\begin{enumerate}
...
\item La fuente predeterminada de los documentos \verb!beamer! es sin serifas,
incluso en ecuaciones matemáticas, si se quiere utilizar la fuente
predeterminada del resto de documentos, debe usarse configurarse usando la
...
salgan en español, debe configurarse el idioma mediante el paquete \verb!babel!,
pero requerimos, además, de dos instrucciones adicionales
    \begin{lstlisting}[language={[LaTeX]TeX},
...
transparencias a dos columnas. Para crear una transparencia de título se hace
de un modo muy parecido a como
...
2 y 3 se especifica cada autor separado por un salto de línea. Los comandos
\verb!\inst{<número>}! permiten que colocar un superíndice con un número que nos
...
con la orden \verb!\frametitle! no son los que aparecen en la tabla de
...
ir fuera del entorno frame.

En algunas transparencias, quizás, querramos destacar un párrafo sobre los
...
En el código \ref{lst:alertblocks} se pueden ver ejemplo de cómo se introducen
